...
import { globals } from "jsr:@denops/std@~7.2.0/variable"
...
export class Config extends BaseConfig {
	override async config(args: {
		...
		const config = await globals.get(args.denops, "config")
		const cache = await globals.get(args.denops, "cache")
		...
		if (tomlExt) {
			const tomlPromises = [
				{ path: `${config}/dpp.toml`, lazy: false },
				{ path: `${config}/ddc.toml`, lazy: true },
				{ path: `${config}/theme.toml`, lazy: true },
				{ path: `${config}/denops.toml`, lazy: true },
			].map((tomlFile) => 
